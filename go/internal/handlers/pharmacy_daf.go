...
func (a *API) enqueueVamregIfNeeded(ctx context.Context, practiceID string, doc store.DAFDocument) {
	if !doc.HasAntibiotic || doc.VamregStatus != "pending" || a.vamregQ == nil {
		return
	}
	if err := a.vamregQ.EnqueueDeclare(ctx, practiceID, doc.ID); err != nil {
		// Leave pending — vet can retry from UI; never silent.
		log.Printf("vamreg enqueue daf=%s: %v", doc.ID, err)
	}
}
