...
func (f *fakeVamregStore) BuildVamregDeclareRequest(ctx context.Context, practiceID, dafID string) (pharmacy.VamregDeclareRequest, error) {
	if f.buildErr != nil {
		return pharmacy.VamregDeclareRequest{}, f.buildErr
	}
...
func TestProcessDeclareMarksFailedOnBuildError(t *testing.T) {
	st := &fakeVamregStore{buildErr: pharmacy.ErrDAFVAMRegIncomplete}
	decl := &pharmacy.VamregDeclarer{
		Client: &pharmacy.VamregClient{DryRun: true},
		Store:  st,
	}
	err := decl.ProcessDeclare(context.Background(), "p", "daf1")
	if err == nil {
	...
	defer st.mu.Unlock()
	if st.status != "failed" {
		t.Fatalf("status=%q want failed (no stuck pending)", st.status)
	}
	if len(st.audits) != 0 {
		t.Fatalf("pre-HTTP failure must not leave started audit, got %v", st.audits)
	}
}
