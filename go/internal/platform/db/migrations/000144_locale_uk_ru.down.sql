...
DECLARE
  cname text;
BEGIN
  -- Reset any uk/ru users before narrowing the CHECK
  UPDATE identity.users SET preferred_locale = 'en' WHERE preferred_locale IN ('uk', 'ru');
...
ALTER TABLE identity.users
  ...
  CHECK (preferred_locale IN ('fr', 'nl', 'en', 'es', 'et', 'it'));
