...
type brandedCustomContent struct {
	Lang             string
	ProductLabel     string
	Tagline          string
	BodyHTML         string // trusted fragment (not escaped)
	Preheader        string
	Disclaimer       string
	FooterPoweredBy  string
	FooterVisit      string
...
// renderBrandedCustomHTML — same shell as renderBrandedEmail with a custom HTML body.
func renderBrandedCustomHTML(c brandedCustomContent) string {
	lang := html.EscapeString(c.Lang)
	if lang == "" {
		lang = "fr"
	}
    ...
    <tr>
      <td align="center" style="padding:40px 16px;">
        <table role="presentation" width="%d" cellspacing="0" cellpadding="0" border="0" style="max-width:%dpx;width:100%%;">
          <tr>
            <td style="background-color:%s;background:linear-gradient(135deg, %s 0%%, %s 100%%);border-radius:12px 12px 0 0;padding:36px 40px;text-align:center;">
              <div style="font-size:30px;font-weight:700;color:#FFFFFF;letter-spacing:-0.03em;line-height:1.2;">%s</div>
              <div style="margin-top:8px;font-size:14px;color:rgba(255,255,255,0.88);">%s</div>
            </td>
          </tr>
          <tr>
            <td style="background-color:%s;padding:40px;border-left:1px solid %s;border-right:1px solid %s;font-size:16px;line-height:1.65;color:%s;text-align:left;">
              %s
            </td>
          </tr>
          <tr>
		...
		colorPrimary, colorPrimary, colorAccent,
		productHTML, tagline,
		colorSurface, colorBorder, colorBorder, colorText,
		c.BodyHTML,
		colorBg, colorBorder,
		colorMuted, disclaimer,
		llitLogoBlock,
