...
func (s *Store) CreateProspect(ctx context.Context, commercialUserID string, in ProspectInput) (Prospect, error) {
	...
	err := s.pool.QueryRow(ctx, `
		INSERT INTO sales.prospects (id, commercial_user_id, practice_name, contact_name, contact_email, contact_phone, city, notes, status)
		VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9)
		RETURNING id::text, commercial_user_id::text, practice_name, contact_name, contact_email, contact_phone, city, notes, status, status_changed_at, created_at`,
		id, commercialUserID, in.PracticeName, in.ContactName, in.ContactEmail, in.ContactPhone, in.City, in.Notes, status).Scan(
		&p.ID, &p.CommercialUserID, &p.PracticeName, &p.ContactName, &p.ContactEmail, &p.ContactPhone, &p.City, &p.Notes, &p.Status, &p.StatusChangedAt, &p.CreatedAt)
	if err != nil {
		return Prospect{}, err
	}
...
func (s *Store) GetProspect(ctx context.Context, commercialUserID, id string) (Prospect, error) {
	var p Prospect
	err := s.pool.QueryRow(ctx, `
		SELECT id::text, commercial_user_id::text, practice_name, contact_name, contact_email, contact_phone, city, notes, status, status_changed_at, created_at
...
func (s *Store) ListAllProspects(ctx context.Context, statusFilter string) ([]Prospect, error) {
	rows, err := s.pool.Query(ctx, `
		SELECT p.id::text, p.commercial_user_id::text, p.practice_name, p.contact_name, p.contact_email, p.contact_phone,
			p.city, p.notes, p.status, p.status_changed_at, p.created_at,
	...
	for rows.Next() {
		...
		if err := rows.Scan(&p.ID, &p.CommercialUserID, &p.PracticeName, &p.ContactName, &p.ContactEmail, &p.ContactPhone,
			&p.City, &p.Notes, &p.Status, &p.StatusChangedAt, &p.CreatedAt, &p.CommercialName, &p.CommercialEmail); err != nil {
			return nil, err
		}
		p.DaysInStatus = daysSince(p.StatusChangedAt)
		out = append(out, p)
...
func scanProspects(rows pgx.Rows) ([]Prospect, error) {
	...
	for rows.Next() {
		var p Prospect
		if err := rows.Scan(&p.ID, &p.CommercialUserID, &p.PracticeName, &p.ContactName, &p.ContactEmail, &p.ContactPhone,
			&p.City, &p.Notes, &p.Status, &p.StatusChangedAt, &p.CreatedAt); err != nil {
			return nil, err
		}
		p.DaysInStatus = daysSince(p.StatusChangedAt)
		out = append(out, p)
