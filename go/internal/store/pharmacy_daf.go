...
func (s *Store) UpsertPracticeMedicationWithdrawal(ctx context.Context, practiceID, medicationID string, meat, milk, eggs *int, banned bool) (RefMedication, error) {
	...
	_, err = s.pool.Exec(ctx, `
		...
		ON CONFLICT (practice_id, medication_id) DO UPDATE SET
			...
			food_chain_banned = EXCLUDED.food_chain_banned,
			updated_at = now()`,
		practiceID, medicationID, meat, milk, eggs, banned)
	if err != nil {
		return RefMedication{}, err
	}
