...
| Rôle | Email | Cabinet |
|------|-------|---------|
| Véto | `vet.demo@petsfollow.test` | Cabinet VetPlus Demo — multi-sites (primary + Antenne Liège) · multi-profil aussi `research` (switch → `/research`) |
...
| Commercial manager | `commercial.manager@petsfollow.test` | Bérénice — équipe démo forcée au seed : Camille + Alex (2 reps) — multi-profils Pro (sauf `admin` / `dev`) |
| Commercial | `commercial.demo@petsfollow.test` | Camille — vet.demo assigné, 5 prospects Bruxelles, rattaché manager — multi-profils Pro (sauf `admin` / manager / `dev`) |
| Commercial | `commercial.demo2@petsfollow.test` | Alex — vet.parc assigné, 5 prospects Nord, rattaché manager — multi-profils Pro (sauf `admin` / manager / `dev`) |

Entraînement pitch IA (commercial) : `/commercial/training` — nécessite `GEMINI_API_KEY`. Admin : `/admin/training`. Module CR IA (**inclus dans Pro**) : `/admin/ai-modules`, `/commercial/ai-modules`, guide `/commercial/ai-cr-playbook` — job quotidien adhésion+friction : `POST /api/v1/internal/ai-module-friction/run` + `X-Ai-Module-Friction-Secret` (`AI_MODULE_FRICTION_SECRET`, scheduler `infra/gcp/setup-ai-module-friction-scheduler.sh`). Analyseur quotidien : `POST /api/v1/internal/pitch-analyzer/run` + header `X-Pitch-Analyzer-Secret`. Purge RGPD 3 ans d'inactivité : `POST /api/v1/internal/retention/run` + header `X-Retention-Secret` (env `RETENTION_PURGE_SECRET`, scheduler quotidien `make gcp-retention-scheduler`) — purge aussi les partages de dossier périmés (lignes + ZIP en bucket) et annule les walk-ins `consultation_session` orphelins (`confirmed`, sans CR persisté, `scheduled_at` ≥ 6 h ; filet au prochain run quotidien). Brouillons SaaS Flux A (C1) — **en sommeil** : l'abonnement Pro se facture hors application, `INVOICING_SAAS_ENABLED` (défaut `false`) fait répondre 404 `invoicing_saas_disabled` aux routes admin `saas-*` et au cron `POST /api/v1/internal/saas-invoices/run` ; scheduler retiré de `setup-all-schedulers.sh` (script `setup-saas-invoices-scheduler.sh` conservé, dormant). Réveil : `INVOICING_SAAS_ENABLED=true` + `SAAS_INVOICES_SECRET` + `make gcp-saas-invoices-scheduler` ; smoke live master `make billit-saas-master-smoke`. Rattrapage des statuts Billit (webhook manqué) : `POST /api/v1/internal/invoicing-reconcile/run` + `X-Invoicing-Reconcile-Secret` (`INVOICING_RECONCILE_SECRET`, scheduler horaire `make gcp-invoicing-reconcile-scheduler`) — relit chez Billit les documents en `sending` depuis plus de 30 min et n'écrit que sur un statut terminal. Auto-branches commerciaux : `POST /api/v1/internal/sales-branches-auto/run` + `X-Sales-Branches-Auto-Secret` (`SALES_BRANCHES_AUTO_SECRET`, scheduler 10h/18h `make gcp-sales-branches-scheduler`) — crée une branche `DUPONTD` pour les commerciaux sans branche et non rattachés à un peer, + email de félicitations. Pharmacie péremption : `POST /api/v1/internal/pharmacy/expiry-run` + `X-Pharmacy-Expiry-Secret` (`PHARMACY_EXPIRY_SECRET`, scheduler quotidien 04:00 `make gcp-pharmacy-expiry-scheduler`) — auto-quarantaine + digest email le lundi. Digest produit quotidien (admin/commercial) : ingest GH Action + `POST /api/v1/internal/product-digest/run` à 18:00 Brussels — voir `documentation/25-PRODUCT-DIGEST.md`.
| Admin | `admin.demo@petsfollow.test` | — (global) — multi-profils Pro complets (admin/dev/research/vet/assist/secretary/commercial/manager) + `client` |
...
**Staging GCP** : pas de seed auto (Scheduler supprimé : `make gcp-delete-seed-scheduler`). Reset manuel : admin Pro (zone danger, phrase `RESET STAGING`) ou `bash infra/gcp/postdeploy.sh --seed`. Annonce staff après seed : email auto / commande `seed-notify`. Au reset : tickets support conservés ; client `b.murgo1976@gmail.com` + graphe propriétaire préservés (remap cabinets démo). Après chaque run du workflow staging : purge auto de **tous** les artefacts smoke/e2e (`make gcp-staging-quality-cleanup`, job `cleanup-quality` en `if: always()` — même si deploy ou Playwright échouent) ; les smoke manuels staging (`gcp-smoke`, `smoke-pharmacy-s6-staging`, `postdeploy.sh`) enchaînent la même purge. Garde-fou couverture : `nuxtjs/tests/unit/e2e-cleanup-coverage.spec.ts`.
...
**Modules Pro tag `dev`** (badge `nav.tagDev`, pas GA) : facturation `/invoicing` (`BILLIT_ENABLED` — cabinet → **ses clients** : particulier par défaut, envoi email `Transporttype: SMTP` ; professionnel = Peppol/SDI. Type mémorisé sur la fiche client (`billingCustomerKind`, migration 000165) ; vide = déduction par identifiant fiscal. Lignes préremplies en fin de consultation via `GET /practices/me/invoicing/prefill?visitId=&dafId=` : acte au tarif du type de RDV (`practice.visit_types`, migration 000166, édité dans `/settings` onglet Agenda) + médicaments du DAF finalisé au prix catalogue. Flux A SaaS LL-IT-SC dormant, `INVOICING_SAAS_ENABLED`) · pharmacie `/medicaments` `/stock` `/daf` (`PHARMACY_ENABLED`) · **AFMPS/Compendium** `/admin/afmps-imports` `/admin/compendium-imports` (même flag, doc [`27-PHARMACIE-BELGIQUE.md`](documentation/27-PHARMACIE-BELGIQUE.md)) · **consignes** `/prescriptions` (`PRESCRIPTIONS_ENABLED`, label UI Consignes — ≠ ordonnance légale ; doc [`35-PRESCRIPTIONS.md`](documentation/35-PRESCRIPTIONS.md)) · **eID BE** préremplissage client (`EID_ENABLED` / `NUXT_PUBLIC_EID_ENABLED`, Redis obligatoire hors local pour Web eID, doc [`46-EID-BELGIQUE.md`](documentation/46-EID-BELGIQUE.md)) · **PACS** onglet Imagerie + `/admin/pacs` (`PACS_ENABLED`, doc [`40-PACS.md`](documentation/40-PACS.md) — local : `make up-pacs` + `make pacs-demo-seed`) · **Research** `/research` + `/admin/research` (`RESEARCH_ENABLED`, doc [`42-RESEARCH.md`](documentation/42-RESEARCH.md) — scheduler `make gcp-research-etl-scheduler`) · **Veille news** dashboard (`VET_NEWS_ENABLED` — ingest Anses/VE/TVP/LPV/WOAH, job `make gcp-vet-news-scheduler`) · **Client AI** Flutter (vulgarisation CR + triage 24/7, `CLIENT_AI_ENABLED` — staging Cloud Run on, prod opt-in ; doc [`43-CLIENT-AI.md`](documentation/43-CLIENT-AI.md)) · **CR IA avancé + RAG** `/admin/rag` (`AI_CR_ADVANCED_ENABLED`, doc [`44-AI-CR-ADVANCED.md`](documentation/44-AI-CR-ADVANCED.md)). Règle `.cursor/rules/modules-tag-dev.mdc`.
...
- **Contact public (staging/prod)** : email `barbara@petsfollow.app` (`SUPPORT_INBOX_EMAIL`) · téléphone `0478.02.33.77` (`COMMERCIAL_CONTACT_PHONE`, repli mail/PDF dossier public si le commercial rattaché n'a pas de `contact_phone`). Source profil commercial toujours prioritaire (porte `/complete-contact-phone` + seed). Les rôles `commercial` / `commercial_manager` y sont redirigés tant qu'ils n'ont pas renseigné le leur (middleware global Nuxt, `PATCH /api/v1/me`).
...
- **Purge RGPD (one-shot ops)** : après premier deploy du câblage secret, exécuter `RETENTION_PURGE_SECRET=… make gcp-retention-scheduler` puis **redeploy** l'API (sinon `pf_api_secrets` ignore le secret absent et `/internal/retention/run` reste en 401). Lifecycle filet : `make gcp-setup-media` (préfixe `dossier-shares/`, 2 jours, **merge** avec les règles existantes). Même convention manuelle pour auth-health / ai-module-friction / saas-invoices / invoicing-reconcile / sales-branches-auto / pharmacy-expiry / **afmps-import** / research-etl / **vet-news** (`SAAS_INVOICES_SECRET=… make gcp-saas-invoices-scheduler`, `SALES_BRANCHES_AUTO_SECRET=… make gcp-sales-branches-scheduler`, `PHARMACY_EXPIRY_SECRET=… make gcp-pharmacy-expiry-scheduler`, `AFMPS_IMPORT_SECRET=… make gcp-afmps-import-scheduler` — CSV pack sur `gs://$GCS_MEDIA_BUCKET/afmps-imports/latest.csv`, gate 1 seule + revue admin, `RESEARCH_ETL_SECRET=… RESEARCH_ANON_SALT=… make gcp-research-etl-scheduler`, `VET_NEWS_SECRET=… make gcp-vet-news-scheduler` puis `--update-secrets` ou redeploy + **first run** `POST /internal/vet-news/run` — pas d’ingest au boot). Le secret du job Scheduler est en clair dans la config HTTP du job : restreindre `scheduler.jobs.get` ; le script n'affiche plus le YAML du job.
- **BFF proxy IP (rate limit Pro)** : secret partagé `BFF_PROXY_SECRET` (`petsfollow-bff-proxy-secret` / prod `petsfollow-prod-bff-proxy-secret`) monté sur **API et Nuxt**. Sans lui, la BFF ne relaie pas `X-PF-Client-IP` (fail-closed) — le rate limit auth Pro tombe sur l'egress Nuxt. One-shot : `make gcp-bff-proxy-secret ATTACH=1` (crée + `--update-secrets` sur les deux services). Local : `make api-dev` / `nuxtjs-dev` utilisent `dev-bff-proxy-secret` + `TRUSTED_PROXY_HOPS=0`.
