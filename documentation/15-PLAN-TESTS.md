...
| D11b | P1 | Compendium PDF | Nav admin → `/admin/compendium-imports` (flag pharmacy) | Page liste + badge `dev` ; extract chunké **persist+resume** (`TestCompendiumImportResumeAfterChunkFailure`) + matching CNK AFMPS + lookup-cnk + pending→confirm→commit (meta manufacturer/substance/**strength**) ; revue UI **dual list** (À contrôler / Prêtes) + **filtres** + **pagination** client + ATC/force + **viewer PDF** (`GET …/pdf`, CSP `frame-src 'self'`) ; Vitest `compendium-review.spec` · DELETE job · e2e `21-compendium-admin` · Go `TestCompendium*` |
| D11c | P1 | Import AFMPS CSV | Nav admin → `/admin/afmps-imports` (flag pharmacy) | Page liste + badge `dev` ; **e2e UI** triple contrôle validate→reviewed→commit (`23-afmps-admin` + fixture `afmps-mini.csv`) ; filtre collisions ; meta JSON merge · Go `TestAFMPSImportTripleGate` / `Gate1Blocked` / `MetaMerge` / `TestInternalAfmpsImportRun*` · unit `pharmacy/afmps_csv_test` · runbook `38` § Import AFMPS · cron mensuel gate 1 |
...
| D14 | P1 | Support inbox | Topbar Support → ticket ; `/admin/support` liste (+ `q` + filtres statut) + détail + **commentaires** + **historique statut** + **PJ** | Ticket visible ; commentaire listé ; statut `open→…→done`/`closed` historisé ; PJ stream auth ; mails création (ops+créateur) / statut (admin+créateur) soft-fail · e2e `14-support.spec.ts` `@p1` · Go search/export/anonymize + `TestSupportTicketStatusWorkflowAndAttachments` + `TestSupportTicketNotifySoftFail` |
...
| C7.21 | P1 | AFMPS cron gate 1 | `POST /internal/afmps-import/run` : secret + objet média → staging + notif ; skip pending/blocked/unchanged/empty/too_large ; jamais commit auto | Go `TestInternalAfmpsImportRun*` ; `make gcp-afmps-import-scheduler` |
| C7.22 | P1 | Sync listes VAMReg readonly | `POST /internal/pharmacy/vamreg-ref-sync` (dry-run défaut, apply atomique, refuse liste vide) → `pharmacy.vamreg_ref_codes` ; `GET …/vamreg-refs?kind=` | Go `TestPharmacyVamregRefSyncDryRunAndApply` / `TestPharmacyVamregRefSyncRejectsEmptyList` / `TestFetchVamregRefLists` ; secret `PHARMACY_VAMREG_REF_SYNC_SECRET` |
| C7.23 | P1 | Immutabilité mouvements + preuve 5 ans | Trigger append-only (+ nullify FK/RGPD) ; `GET …/movements/retention-stats` (`immutableEnforced`) | Go `TestPharmacyStockMovementsImmutableAndRetentionStats` ; DAF UI `vamreg-refs` (datalist) |
| C7.24 | P1 | Dictionnaire `/medicaments` A–Z + liaison AFMPS | `GET …/medications?letter=` (+ `letterCounts`/`catalogTotal`, `includeCounts=0` pagination) ; fiche expose `afmpsMeta` / `afmpsSource` ; UI index A–Z + badges source | Go `TestPharmacyMedicationListByLetter` ; Vitest `medicaments-alphabet.spec.ts` |
