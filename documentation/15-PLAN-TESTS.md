...
| C7.21 | P1 | Sync listes VAMReg readonly | `POST /internal/pharmacy/vamreg-ref-sync` (dry-run défaut, apply atomique, refuse liste vide) → `pharmacy.vamreg_ref_codes` ; `GET …/vamreg-refs?kind=` | Go `TestPharmacyVamregRefSyncDryRunAndApply` / `TestPharmacyVamregRefSyncRejectsEmptyList` / `TestFetchVamregRefLists` ; secret `PHARMACY_VAMREG_REF_SYNC_SECRET` |
| C7.22 | P1 | Immutabilité mouvements + preuve 5 ans | `REVOKE UPDATE/DELETE` `stock_movements` (RGPD via `rgpd_null_stock_movement_created_by`) ; `GET …/movements/retention-stats` | Go `TestPharmacyStockMovementsImmutableAndRetentionStats` ; DAF UI consomme `vamreg-refs` (datalist) |
