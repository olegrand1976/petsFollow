...
**PACS Orthanc (tag `dev`)** : status/wake + upload `.dcm` (magic `DICM`) + viewer fiche animal + admin `/admin/pacs` sous `PACS_ENABLED` — Go `TestPacs*` (upload · preview/file via `ParentSeries` fallback · isolation cross-cabinet `vet.parc` 404 opaque · client 403 · ID Orthanc invalide 400) · Vitest `pacsPoll` · Playwright `@p0` [`20-pacs-admin.spec.ts`](../nuxtjs/tests/e2e/specs/20-pacs-admin.spec.ts) + [`20b-pacs-imaging.spec.ts`](../nuxtjs/tests/e2e/specs/20b-pacs-imaging.spec.ts) (tab + **upload fixture + canvas non vide** si flag on) · doc [40](40-PACS.md) (checklist GA). Staging : Orthanc dans Cloud Build + `PACS_ORTHANC_URL` auto. Pas de useCase commercial tant que tag `dev`.
