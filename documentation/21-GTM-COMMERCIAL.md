# GTM commercial — petsFollow

Aligné fiches [18](18-FICHE-COMMISSION-VETO.md) / [19](19-FICHE-COMMISSION-COMMERCIAL.md) / politique [17](17-POLITIQUE-TARIFAIRE.md).

## Rôles
| Rôle | Mission | KPI |
|------|---------|-----|
| Commercial (apporteur) | Ouvrir cabinets + activer pets payants | Pets payants / mois · mix triennial ≥ 55 % |
| Véto (prescripteur) | Prescrire l’abo au client | Clients payants · progression paliers |

## Playbook 30 jours
1. Onboard véto (profil cabinet complet)
2. Pitch triennial (~2,6 €/mois · ~9,4 € commission chacun @ plafond)
3. 10 clients prescrits / 30 j
4. Family / Kennel / Care+ / Horse = **abos annuels récurrents** ; Family dès 2 (−10 %, pas de plafond) ; Kennel dès 6 (−15 %, encodage) — **exclusifs** (Kennel = upgrade Family)
5. Care+ après usage rappels

## Scripts courts
- **Client** : « Suivi prescrit par votre véto, sans boîtier — moins de 3 €/mois. »
- **Véto** : « Vous gagnez sur chaque activation ; plafond ~9,4 € sur le triennial ; pas de perte si un commercial vous a apporté. »
- **Commercial** : « Ouvrez le cabinet, activez 5 pets en 60 j → bonus 25 € + commission sur chaque nouvelle activation. »

## SPIFF
- Ramp 25 € · Mix mois 50 € : détection auto + mark-paid admin
- Palier véto 50 € : affichage / payout hors système

## Interdits
- Compter une commission (ou un « deal gagné ») dès l’inscription du véto — un cabinet commence à **0** payant ; revenu = animal payant
- Promettre un % calculé sur le **TTC** (assiette = **HTVA**)
- Dire au véto qu’il gagne moins s’il y a un commercial (grilles indépendantes)
- Détail → [22-FICHE-PRODUIT-COMMERCIAL.md](22-FICHE-PRODUIT-COMMERCIAL.md)
