...
export default defineEventHandler(async (event) => {
  ...
  if (typeof q.clientUserId === 'string' && q.clientUserId) params.set('clientUserId', q.clientUserId)
  if (typeof q.vetUserId === 'string' && q.vetUserId) params.set('vetUserId', q.vetUserId)
  const qs = params.toString()
  return proxyApi(event, `/api/v1/admin/filiation/events${qs ? `?${qs}` : ''}`)
})
