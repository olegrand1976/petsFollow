...
test('responsable commercial voit suivi et prospects équipe', async ({ page }) => {
  ...
  await expect(page.getByTestId('manager-overdue-tasks')).toBeVisible()
  await page.goto('/commercial-manager/agenda')
  await expect(page.getByTestId('manager-agenda-page')).toBeVisible()
