...
test('pet detail — tension + panel labo @p1', { tag: '@p1' }, async ({ page }) => {
  ...
  // Seed densify can leave duplicate BP labels in the card — pin the first match.
  await expect(vitals.getByTestId('pet-bp-table-card').getByText('142/91').first()).toBeVisible()
...
test('pet detail — données médicales + statut animal', { tag: '@p1' }, async ({ page }) => {
  ...
  const clearLifecycle = () => fetch(`${API}/api/v1/vet/pets/${petId}/lifecycle`, {
    method: 'PATCH',
    headers: {
      Authorization: `Bearer ${vetTok}`,
      'Content-Type': 'application/json',
