    ...
    <div class="pro-login-form-panel">
      ...
      >
        ...
        <ProInput
          ...
          name="inviteCode"
          autocomplete="off"
          test-id="register-invite-code"
        />
        <p class="pro-field-hint">{{ $t('auth.register.inviteCodeHint') }}</p>
        <p v-if="!inviteCode.trim()" class="pro-field-hint" data-testid="register-pool-hint">
          {{ $t('auth.register.poolWithoutCode') }}
        </p>
...
const inviteCode = ref(String(route.query.invite || '').trim())
const error = ref('')
